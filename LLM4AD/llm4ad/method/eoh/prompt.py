...
class EoHPrompt:
    @classmethod
    def _build_method_requirement(
        cls,
        method_name: str,
        method_usage: str,
    ) -> str:
        usage = method_usage or "Make this method compatible with the Algorithm class."
        return f"You need to implement the method '{method_name}' within it, {usage}\n"
    ...
    def _maybe_context_block(
        ...
        if not use_context:
            return ""
        return cls._build_context_block(
            method_introduction,
            main_stream,
        )
    ...
    def get_prompt_i1(cls, task_prompt: str, template_function: Function, method_name: str, method_usage: str,
                      method_introduction: str, main_stream: str | None = None, use_context: bool = True):
        # template
        temp_func = copy.deepcopy(template_function)
        temp_func.body = ''
        # create prompt content
        method_block = cls._build_method_requirement(method_name, method_usage)
        context_block = cls._maybe_context_block(
            use_context, method_introduction, main_stream
        )
...
{func_template}
1. First, Use a sentence inside the {{}} after 'thought' to describe your algorithm. 
2. Next, implement the following Python function after 'code':
{str(temp_func)}
...
1. Second, Use a sentence inside the {{}} after 'thought' to describe your algorithm. 
2. Lastly, implement the following Python function after 'code':
{str(temp_func)}
Do not give additional explanations.'''
        return prompt_content
